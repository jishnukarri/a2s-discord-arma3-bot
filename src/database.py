...
class BotDatabase:
    def __init__(self, database_file: str):
        ...
        if os.path.exists(database_file) != True:
            # leaderboard db
            self.cursor.execute(
                "CREATE TABLE database (player_name TEXT, kills INTEGER,playtime INTEGER, month TEXT, PRIMARY KEY (player_name,month))"
            )
            # message db
            self.cursor.execute(
                "CREATE TABLE messages (id INTEGER, leaderboard INTEGER, status INTEGER, PRIMARY KEY(id))"
            )
            self.cursor.execute(
                "INSERT INTO messages(id,leaderboard,status) VALUES (1,0,0)"
            )
    ...
    def updateMessageID(self, messages) -> None:
        if (messages[1] == 000) and (messages[2] == 000):
            ...
            logger.info(
                f"Message ID updated - leaderboard: {messages[1]},status: {messages[2]}"
            )
            query = (
            ...
            self.cursor.execute(
                query, (messages[1], messages[2], messages[1], messages[2])
            )
            self.database.commit()

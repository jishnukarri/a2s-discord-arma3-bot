...
class BotDatabase:
    def __init__(self,database_file:str):
        ...
        if os.path.exists(database_file) != True:
            ...
            self.cursor.execute("INSERT INTO messages(id,leaderboard,status) VALUES (1,0,0)")
    ...
    def getMessageID(self) -> dict:
        ...
        if (messages[1] == 0) or (messages[2] == 0):
            ...
            logger.info(f"message id exists - leaderboard: {messages[1]}status:{messages[2]}")
    ...
    def updateMessageID(self,messages) -> None:
        if (messages[1] == 000) and (messages[2] == 000):
            ...
            logger.info(f"Message ID updated - leaderboard: {messages[1]},status: {messages[2]}")
            query = "INSERT INTO messages(id,leaderboard,status) VALUES(1,?,?)" "ON CONFLICT(id) DO UPDATE SET leaderboard=?,status=?;"
            self.cursor.execute(query,(messages[1],messages[2],messages[1],messages[2]))
            self.database.commit()
    ...
    def updateLeaderboard(self,player_name:str, kill_increase:int, playtimeIncrease:int) -> None:
        ...
        self.cursor.execute("INSERT INTO database(player_name, kills, time, month) VALUES(?,?,?,?)"
                    "ON CONFLICT(player_name,month) DO UPDATE SET kills = kills + ?, playtime = playtime + ?;",
                    (player_name,kill_increase,playtimeIncrease,current_time,kill_increase,playtimeIncrease))
        self.database.commit()
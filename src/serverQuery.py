...
class a2sQuery():
    def __init__(self,serverTuple:serverTuple,rQuery:int,server_name:str):
        try:
            ...
            logger.error(f'SERVER_TUPLE FOR {serverTuple} has been configured incorrectly. \n Format : ("SERVER_HOST",SERVER_PORT)')
    ...
    def getServerInfo(self):
        if (time.time() - self.serverInfo[1] > 10):
            try:
                ...
                logger.error(f'Unable to fetch server info for {self.serverTuple} \n Error: {e}')
            self.serverInfo[1] = time.time() # This run wheter the query failed or not to avoid requests on servers which are offline
    ...
    def getServerRules(self):
        if (time.time() - self.rules[1] > 10):
            try:
                ...
                logger.error(f'Unable to fetch server rules for {self.serverTuple} \n Errpr: {e}')
